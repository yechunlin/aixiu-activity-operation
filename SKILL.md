...
1. 将用户 `brief` 中的**所有实义词**（去除"的、了、是、在、有、和、就、不、人、都、一、一个、上、也、很、到、说、要、去、你、会、着、没有、看、好、自己、这"等停用词）提取为关键词集合
...
- edit_step_2 和 edit_step_3 的第一次循环在语义上等价
- 用户可能在 edit_step_2 回答中同时提多个修改需求（如"改标题，再加个手机号字段"），edit_step_3 需一次性处理
...
以下 Prompt 用于验证 Skill 效果，每次优化后应重新跑这些用例。

### 创建活动

1. **基础创建**："帮我创建一个报名活动，标题是「2026春季亲子运动会」，活动介绍是「欢迎各位家长带孩子参加春季亲子运动会」。"
2. **带字段+抽奖**："创建一个报名活动，主题是环保志愿者招募，需要收集姓名、手机号、紧急联系人，还要关联抽奖。"
3. **指定海报**："创建一个报名活动，标题「读书分享会」，我有海报图，链接是 https://example.com/poster.jpg。"

### 编辑活动

4. **修改标题**："帮我编辑活动，活动ID是 abc123def456，把标题改成「2026春季亲子运动会（正式版）`。"
5. **修改字段**："帮我编辑活动，活动ID是 abc123，加一个"年龄"字段，类型是数字。"
6. **删除字段**："帮我编辑活动，活动ID是 abc123，删除"紧急联系人"字段。"

### 边界条件

7. **无效活动ID**："帮我编辑活动，活动ID是 123，改成啥？"
8. **模糊标题**："帮我创建一个活动。"（无具体信息）
9. **重复字段**："帮我编辑活动，活动ID是 abc123，加一个"姓名"字段。"（字段已存在）

### 多轮对话

10. **逐步补充信息**："创建一个报名活动。" → （回复）"标题是「环保志愿者招募」。" → （回复）"需要收集姓名、手机号。"
11. **中途修改**："创建一个报名活动，标题「测试」。" → （回复）"不对，标题改成「正式活动」。"
12. **多轮编辑**："帮我编辑活动，活动ID是 abc123。" → （回复）"改标题。" → （回复）"改成「新标题」。"

### 语义歧义

13. **歧义字段类型**："加一个"职业"字段。"（应识别为 Text 类型）
14. **歧义抽奖意图**："这个活动有抽奖吗？"（应查询 is_raffle 状态，而非直接修改）
15. **歧义编辑对象**："帮我改一下。"（应询问具体要修改什么）
